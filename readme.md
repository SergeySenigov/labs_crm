...
При появлении в очереди сообщений `crm_activity` сообщения с темой `client_request_new` или `manager_request_new`:

Если поле сообщения `inn` пустое или не соответствует длине ИНН (10 или 12 символов), то 

Если поле сообщения `check_inn_user_id` пустое, то получить из таблицы настроек (предполагаем ее наличие) значение `user_id` менеджера, ответственного за уточнение ИНН клиента.

Если поле сообщения client_request_id пустое, то создать в таблице `Заявки клиентов` новую запись со значениями:
...
Если поле сообщения `client_request_id` не пустое, то найти в таблице `Заявки клиентов` по ключу `client_request_id` соответствующую запись и обновить значения полей:

|Поле БД|Значение|
|man_inn_update_dt| Поле man_inn_update_dt сообщения|
...
|----------|---------|
|client_request_id|`client_request_id` созданной записи или сообщения, если оно заполнено|
|fio|Поле fio сообщения|
|phone|Поле phone сообщения|
|inn|Поле inn сообщения|
|create_dt|Поле create_dt сообщения|
...
[Конец Если поле сообщения `inn` пустое или не соответствует длине ИНН (10 или 12 символов)]
...
Если такая запись найдена, то получить значение `contractor_id` найденной записи.
...
Если поле сообщения `client_request_id` не пустое, то найти в таблице `Заявки клиентов` запись с соответствующим ключем `client_request_id`
и обновить значение полей:
|Поле БД|Значение|
|contractor_id|Поле contractor_id найденной записи|
|status|'inn_found_in_crm'|
...
При внесении менеджером данных в поле `ИНН`, в очередь сообщений `crm_activity` поместить новое сообщение с темой `manager_request_new` с данными:
|Поле сообщения|Данные|
|----------|---------|
...
|man_inn_update_dt|текущее время|

**Задание программисту бизнес-логики CRM**

1. Сервис получения данных по ИНН
2. Сервис информирования менеджера по неверному ИНН (+)
3. Сервис информирования менеджера по выбору ИНН


|Типы сообщений|Отправляет|Получает|
|----------|---------|-----|
|`client_request_new`|V|V|
|`manager_request_new`|V|V|
|`request_ext_info_by_inn`|V|сделать|
|`reply_ext_info_by_inn`|сделать|V|
|`request_manager_check_inn`|V|V|
