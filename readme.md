...
* если reply_status = found_ul, то создать в таблице `Информация о контрагентах` новую запись со значениями:
...
создать в таблице `Информация о контрагентах` новую запись со значениями:
...
4. Форма (+)
5. Таймер для повтора внешнего опроса


|Типы сообщений|Отправляет|Получает|
|----------|---------|-----|
|`client_request_new`|V|V|
|`request_manager_validate_inn`|V|V|
|`request_ext_info_by_inn`|V|сделать внешний сервис|
|`reply_ext_info_by_inn`|сделать внешний|V|
